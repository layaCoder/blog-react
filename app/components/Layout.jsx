...
class LayoutComponent extends Component {
  ...
  componentDidMount() {
    ...
    if (
      this.props.location.pathname.includes("blogall") &&
      this.props.location.pathname.indexOf("blogfilter") === -1
    ) {
      ...
      let tag = this.props.location.pathname.substring(
        index + 1,
      ...
      let serachParam = this.props.location.pathname.substring(
        paramStrIndex + 1,
        this.props.location.pathname.length
      );
    ...
    if (getLocalStorage("user", 1000 * 60 * 60 * 24) !== null) {
      ...
      this.props.dispatch(
        userLogin(
          true,
          JSON.parse(getLocalStorage("user", 1000 * 60 * 60 * 24)).name,
          JSON.parse(getLocalStorage("user", 1000 * 60 * 60 * 24)).avatar
        )
      );
    } else {
  ...
  initMyBlogData = () => {
    if (!getLocalStorage("user", 1000 * 60 * 60 * 24)) {
      ...
      let url =
        APIS.blogList.devUrl + "?pageIndex=1&pageSize=10&user=" + user.name;
  ...
  handleSearch = (value) => {
    ...
    let url =
      APIS.blogListBySearch.devUrl + "?pageIndex=1&pageSize=10&param=" + value;
    ...
    return (
      <div>
        <AsideMenu
          ...
          toggleAside={this.showAside}
          onClick={this.hideCover}
        />
        <div
          className={this.state.showAside ? "mobile-aside-wrap" : ""}
          onClick={this.hideCover}
        >
          <Layout className="layout">
            <Header style={{ position: "fixed", zIndex: 1, width: "100%" }}>
              <Row>
                  ...
                  >
                    <Menu.Item key={["/app/blogall"]}>
                      <Link
                        to={{
                          pathname: "/app/blogall",
                          state: this.props.store.blogs,
                        }}
                        onClick={this.initBlogAllData}
                      >
                        All Blogs
                      </Link>
                    </Menu.Item>
                    <Menu.Item
                      key={["/app/myblog"]}
                      style={{
                        display: this.state.isLogin == true ? "" : "none",
                      }}
                      onClick={this.initMyBlogData}
                    >
                    ...
                    <Menu.Item key={["4"]} style={{ display: "none" }}>
                      <Link to={"/app/blogall/blogfilter"}>
                        Blog Filter By Tag
                      </Link>
                    </Menu.Item>
                  </Menu>
                ...
                <Col
                  span={3}
                  style={{ display: this.state.isLogin === true ? "none" : "" }}
                >
                ...
                >
                  ...
                  <Dropdown overlay={menu}>
                    <a className="ant-dropdown-link" href="#">
                      <h3 className="head-userName">
                        {" "}
                        Hello:{" "}
                        {this.props.store.isLogin.login === true
            ...
            {/* 进度条 */}
            {this.state.ProgressPercent === 100 ? null : (
              <Progress
            ...
            <Row style={{ marginTop: "60px" }}>
              <Col
                span={this.state.isPc ? 12 : 22}
                offset={this.state.isPc ? 3 : 1}
              >
                <Content>
                  {/* <BreadcrumbCusstom /> */}
                  <div
                    ...
                    {this.props.store.showLoading === false ? (
                      <Switch>
                        ...
                        <Route
                          ...
                          component={WriteBlog}
                        />
                        <Route exact path="/app/blogall" component={BlogAll} />
                        <Route
                          exact
                          path="/app/blogall/blogdetail/:id"
                          component={BlogDetail}
                        />
                        <Route
                          exact
                          path="/app/blogall/blogfilter/:tag"
                          component={BlogFilterByTags}
                        />
                        <Route
                          exact
                          path="/app/blogbysearch/:param"
                          component={BlogBySearch}
                        />
                        <Route
                          exact
              ...
              {this.state.isPc && (
                <Col span={7} className="rightMenu">
                  <Row>
                    <div>
                      {/* 标签栏 */}
                      <TagsBoard />
            ...
            <div>
              ...
              >
                <Login
                  ...
                  showLoginRoot={this.showLoginRoot}
                />
              </Modal>
            </div>
            <div>
              ...
              <Modal
                ...
                closable={false}
              >
                {this.state.uploadAvatarVisible === true ? (
                  <UploadAvatarComponent
                    handleClose={this.handleCloseUploadAvatar}
                  />
                ) : null}
              </Modal>
            </div>
            <div>
              ...
              >
                {this.state.changePassVisible === true ? (
                  <ChangePass
                    handleCloseChangePass={this.handleChangePassClose}
                  />
                ) : null}
              </Modal>
            </div>
